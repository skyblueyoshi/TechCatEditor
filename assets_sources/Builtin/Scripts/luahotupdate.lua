...
function HU.InitFileMap(RootPath)
    for _, rootpathinfo in pairs(RootPath) do
        local modulename = rootpathinfo[1]
        local rootpath = Path.fix(rootpathinfo[2])
        local tmpList = AssetManager.getAllFiles(rootpath, ".lua", false, true, true)
        for _, path in ipairs(tmpList) do
            local FileName = Path.getFileNameWithoutExtension(path)
            if HU.FileMap[FileName] == nil then
                HU.FileMap[FileName] = {}
            end
            local luapath = Path.getRelativePath(rootpath, path)
            luapath = luapath:sub(0, #luapath - 4)
            luapath = string.gsub(luapath, "/", ".")
            luapath = modulename .. "." .. luapath
            HU.LuaPathToSysPath[luapath] = SysPath
            --print("FileName: " .. FileName .. "  path: " .. path .. "  luapath: " .. luapath)
            table.insert(HU.FileMap[FileName], { SysPath = path, LuaPath = luapath })
        end
    end
end
...
function HU.AddFileFromHUList()
    ...
    local FileList = { "_ALL_" }--require (HU.UpdateListFile)
    HU.ALL = false
    HU.HUMap = {}
...
function HU.BuildNewCode(SysPath, LuaPath)
    local NewCode = __get_code(LuaPath)
    --local NewCode = AssetManager.readAsString(SysPath)
    ...
    print(string.format("Hotfix updating: %s %s", LuaPath, SysPath))
    ...
    local function f(t)
        ...
        elseif type(t) == "table" then
            ...
            for k, v in pairs(t) do
                if (k == "require" or v == "require") then
                    print("fccccccccccccccccccccccccccccccccccccccccccccccccc")
                end
                if (k ~= "require" and v ~= "require") then
                    f(k);
                    f(v);
...
function HU.HotUpdateCode(LuaPath, SysPath)
    ...
    elseif HU.OldCode[SysPath] == nil then
        --HU.OldCode[SysPath] = AssetManager.readAsString(SysPath)
        HU.OldCode[SysPath] = __get_code(LuaPath)
    end
end
...
function HU.UpdateUpvalue(OldFunction, NewFunction, Name, From, Deepth)
    ...
    --print(Deepth .. "HU.UpdateUpvalue " .. Name .. "  from:" .. From)
    local OldUpvalueMap = {}
    local OldExistName = {}
    for i = 1, math.huge do
        ...
        --print(Deepth .. "upvalue old check " .. name)
        OldUpvalueMap[name] = value
        if name == "__CODE_INFO__" then
            --print(value)
        end
    ...
    for i = 1, math.huge do
        ...
        print(Deepth .. "upvalue check " .. name)
...
function HU.UpdateOneFunction(OldObject, NewObject, FuncName, OldTable, From, Deepth)
    ...
    print(Deepth .. "HU.UpdateOneFunction " .. FuncName .. "  from:" .. From)
...
function HU.UpdateAllFunction(OldTable, NewTable, Name, From, Deepth)
    ...
    print(Deepth .. "HU.UpdateAllFunction " .. Name .. "  from:" .. From)
