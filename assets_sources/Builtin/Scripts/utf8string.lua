...
utf8string.map = function(s, f, no_subs)
    local i = 0
...
utf8string.chars = function(s, no_subs)
    return coroutine.wrap(function()
        return utf8string.map(s, coroutine.yield, no_subs)
    end)
end

-- returns the number of characters in a UTF-8 string
utf8string.len = function(s)
    -- count the number of non-continuing bytes
    return select(2, s:gsub('[^\128-\193]', ''))
end

-- replace all utf8 chars with mapping
utf8string.replace = function(s, map)
    return s:gsub(pattern, map)
end

-- reverse a utf8 string
utf8string.reverse = function(s)
...
utf8string.sub = function(s, i, j)
    local l = utf8string.len(s)
    ...
    local diff = j - i
    local iter = utf8string.chars(s, true)
