---@class TCE.ScrollBarView:TCE.BaseView
local ScrollBarView = class("ScrollBarView", require("BaseView"))
local UIUtil = require("core.UIUtil")
local Constant = require("config.Constant")

function ScrollBarView:__init(key, parent, parentRoot, isVertical)
    ScrollBarView.super.__init(self, key, nil, parent, parentRoot)
...
function ScrollBarView:_onDestroy()
    self._panelList = nil
    ScrollBarView.super._onDestroy(self)
end

function ScrollBarView:_initContent()
...
function ScrollBarView:adjustLayout(isInitializing, _)
    local margins
    ...
    self._root = UIUtil.ensurePanel(self._parentRoot, self._name, { 0, 0, Constant.SCROLL_BAR_WIDTH, Constant.SCROLL_BAR_WIDTH }, {
        margins = margins, bgColor = "A2", })
    UIUtil.ensurePanel(self._root, "mask", { 0, 0, 12, 12 }, {
        bgColor = "A", margins = { 6, 12, 6, 12, true, true }, })
...
function ScrollBarView:_onScrollDataChanged()
...
function ScrollBarView:_setRate(rate)
    if self._isVertical then
...
function ScrollBarView:_setFull()
    self._slider.height = self._root.height
    self:_setRate(0)
end

function ScrollBarView:_refresh()
...
function ScrollBarView:_moveByMouse()
...
---_onTouchDown
---@param touch Touch
function ScrollBarView:_onTouchDown(_, touch)
...
function ScrollBarView:_onTouchMove(_, touch)
    self._curTouchPos = touch.position
    self:_moveByMouse()
end

function ScrollBarView:_onTouchUp(_, _)
...
function ScrollBarView:setVisible(visible)
